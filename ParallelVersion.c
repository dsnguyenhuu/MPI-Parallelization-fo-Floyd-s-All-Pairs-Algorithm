...
Assignment 3 - Floyd All-Pairs Shortest Path Algorithm - Pipeline
...
int main(int argc, char *argv[]) {
	...
	MPI_Comm_rank(rowComms,&rowRank);
	MPI_Comm_size(rowComms,&rowSize);
	MPI_Comm_rank(colComms, &colRank);
	MPI_Comm_size(colComms, &colSize);
	...
	for (k = 0; k < numNodes; k++) {
		...
		// If a match is found with the y coordinate, then a k needs to be sent through MPI
		...
		MPI_Request request;
		int a;
		for (a = 0; a < sqrtNumTasks; a++) {
			if (a == position[1]) {
				continue;
			}
			MPI_Isend(rowBuffer, rowColNum, MPI_INT,a,k,colComms,&request)
		}

		// MPI Column broadcast 
		MPI_Bcast(rowBuffer, rowColNum, MPI_INT, kFound, colComms);

		//If a match is found with the x coordinate, then a k needs to be sent through MPI
		...
		int b;
		for (b = 0; b < sqrtNumTasks; b++) {
			if (b == position[0]) {
				continue;
			}
			MPI_Isend(colBuffer, rowColNum, MPI_INT, b, k, rowComms, &request);
		}

		if (position[1] != kFound) {
			MPI_Recv(rowBuffer, rowColNum, MPI_INT, kFound,k, colComms, MPI_STATUS_IGNORE);
		}
		if (position[0] != kFound) {
			MPI_Recv(colBuffer, rowColNum, MPI_INT, kFound, k, rowComms, MPI_STATUS_IGNORE);
		}
